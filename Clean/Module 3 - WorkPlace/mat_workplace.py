...
maternelles = pd.read_csv('mat_workid_quartier.csv')

maternelles.drop(columns=["Code_x", "Code_y", "Territoire", "Commune_y", "Unnamed: 0"], inplace=True)
maternelles.rename(columns={"Commune_x":"Commune"}, inplace=True)
print(maternelles)

perc_out = pd.read_csv('mat_from_quart_to_out_bxl_perc.csv', sep=';')
perc_out.drop(columns=["Code"], inplace=True)
perc_out.set_index("Territoire", inplace=True, drop=True)
print(perc_out)

nb_mat_quart = pd.read_csv("nb_inscrits_mat_habitant_le_quartier.csv")
...
for quart in nb_mat_quart_out.index:
    print("quart", quart)
    p = perc_out.loc[quart, "Part des enfants du territoire (quartier ou commune) inscrits dans une école maternelle située en dehors de la Région (%)"]
    if type(p)==str:
         p = p.replace(",", ".")
    p = float(p)/100
    
    if quart in nb_mat_quart.index:
        n = nb_mat_quart.loc[quart, "Nb d'enfants inscrits en maternelles habitant le quartier"]
        if type(n)==str:
            n = n.replace(",", ".")
        n = float(n)
    else:
...
nb_mat_quart_out.to_csv('nb_mat_quart_out.csv')
...
print(mat_out)
mat_out.to_csv('mat_out.csv')

places_mat_per_child = pd.read_csv('places_mat.csv', sep=";") #places par quart
...
for quart in nb_mat_quart_out.index:
    print("quart", quart)
    p = places_mat_per_child.loc[quart, "Capacité d’accueil scolaire relative du territoire (maternel) (nb d'élèves/nb enfants)"]
    if type(p)==str:
         p = p.replace(",", ".")
    p = float(p)
...
nb_mat_quart_in.to_csv('nb_mat_quart_in.csv')


ratio = pd.read_csv("ratio_inscrits_mat_ss_quart.csv", sep=";")
...
for ss in capa_per_ss.index:
    ...
    nb =  round(p*r)
    capa_per_ss.loc[ss, "Places Mat ss"] = nb

print(capa_per_ss)
capa_per_ss.to_csv('capa_per_ss_mat.csv')
...
mat_limitrophes = pd.read_csv('mat_limitrophe_quartier.csv', sep=";") #TODO create file
...
for quart in nb_mat_quart_out.index:
    print(quart, "quart")
    nb_to_limitrophe = mat_limitrophes.loc[quart, "nb_child_vivant_quart_mat_lim"]
    ...
    places_mat_poss = places_mat_poss[places_mat_poss["Places Mat ss"] > 0]
    ...
    while nb_to_limitrophe > 0 and len(possible_child) > 0 and len(possible_ss_to) > 0:
        ...
        places_mat_poss.loc[work_ss_id, "Places Mat ss"] = places_mat_poss.loc[work_ss_id, "Places Mat ss"] - 1
        places = places_mat_poss.loc[work_ss_id, "Places Mat ss"]
        if type(places)!= np.float64:
            places = places[0]
        capa_per_ss.loc[work_ss_id, "Places Mat ss"] = places
...
print(mat_lim)
mat_lim.to_csv('mat_limitrophe.csv')
print(capa_per_ss)
mat_workplace = pd.concat([mat_out, mat_lim], ignore_index = True)

from_to_commune = pd.read_csv("mat_com_flow.csv", sep=';')
...
print(mat_com)
mat_com.to_csv("mat_commune.csv")
...
capa_per_ss = capa_per_ss[capa_per_ss["Places Mat ss"] > 0]
mat_random =  pd.DataFrame(columns=colnames)
i = 0
if not maternelles.empty and not capa_per_ss.empty:
    possibles_ss = capa_per_ss[capa_per_ss["Places Mat ss"] > 0]
    ...
    while len(possibles_ss) > 0 and len(maternelles) > 0:
        ...
        capa_per_ss.loc[work_ss_id, "Places Mat ss"] = capa_per_ss.loc[work_ss_id, "Places Mat ss"] - 1
        places = capa_per_ss.loc[work_ss_id, "Places Mat ss"]
...
print(mat_workplace)
mat_workplace.to_csv("maternelles_workplace.csv")
        
print(capa_per_ss)
capa_per_ss.to_csv("mat_places_restantes_for_foreigners.csv")
