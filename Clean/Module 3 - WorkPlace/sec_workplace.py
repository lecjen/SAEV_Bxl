...
for quart in nb_mat_quart_out.index:
    print("quart", quart)
    p = perc_out.loc[quart, "Part des enfants du territoire (quartier ou commune) inscrits dans une école seco0aire située en dehors de la Région (%)"]
    if type(p)==str:
         p = p.replace(",", ".")
    p = float(p)/100
    
    if quart in nb_mat_quart.index:
        n = nb_mat_quart.loc[quart, "Nb d'enfants inscrits en secondaires habitant le quartier"]
        if type(n)==str:
            n = n.replace(",", ".")
        n = float(n)
    else:
...
nb_mat_quart_out.to_csv('nb_sec_quart_out.csv')
...
print(mat_out)
mat_out.to_csv('sec_out.csv')


"""
places_mat_per_child = pd.read_csv('places_sec.csv', sep=";") #places par quart
...
for quart in nb_mat_quart_out.index:
    print("quart", quart)
    p = places_mat_per_child.loc[quart, "Capacité d’accueil scolaire relative du territoire (secondaire) (nb d'élèves/nb enfants)"]
    if type(p)==str:
         p = p.replace(",", ".")
    p = float(p)
...
nb_mat_quart_in.to_csv('nb_sec_quart_in.csv')


ratio = pd.read_csv("ratio_inscrits_sec_ss_quart.csv", sep=";")
...
capa_per_ss["Places Sec ss"]=np.zeros(len(ratio.index))
...
for ss in capa_per_ss.index:
    ...
    nb =  round(p*r)
    capa_per_ss.loc[ss, "Places Sec ss"] = nb

print(capa_per_ss)
capa_per_ss.to_csv('capa_per_ss_sec.csv')
capa_per_ss["SectorStatID2"]=capa_per_ss.index
#capa_per_ss.reset_index(inplace=True)
"""

capa_per_ss = pd.read_csv('places_sec.csv', sep=";")
...
capa_per_ss=comma_to_dot(capa_per_ss)
capa_per_ss["places_ss"]=capa_per_ss.places_ss.apply(lambda x: round(float(x)))
capa_per_ss.set_index("SectorStatID", inplace=True, drop=True)
...
mat_limitrophes = pd.read_csv('sec_limitrophe_quartier.csv', sep=";") #TODO create file
...
corres_quartier_ss = capa_per_ss
corres_quartier_ss.drop(columns=['nb_sec_com_pop', 'nb_sec_ss_pop', 'ratio_com_ss', 'places_com', 'places_ss'], inplace=True)
...
for quart in nb_mat_quart_out.index:
    print(quart, "quart")
    nb_to_limitrophe = mat_limitrophes.loc[quart, "nb_child_vivant_quart_sec_lim"]
    ...
    places_mat_poss = capa_per_ss.merge(possible_ss_to, how='inner', left_index=True, right_on="Code")
    places_mat_poss = places_mat_poss[places_mat_poss["places_ss"] > 0]
    places_mat_poss.drop_duplicates(inplace=True)

    possible_ss_to = places_mat_poss.index #TODO check index has to be ss id
    #places_mat_poss.set_index("SectorStatID", inplace=True, drop=True)
    ...
    while nb_to_limitrophe > 0 and len(possible_child) > 0 and len(possible_ss_to) > 0:
        ...
        #ind_elu_ss = possible_ss_to[elu_ss]

        work_ss_id =  possible_ss_to[elu_ss] #closest_peri.loc[maternelles.loc[ind_elu, "SectorStatID"]]
        ...
        places_mat_poss.loc[work_ss_id, "places_ss"] = places_mat_poss.loc[work_ss_id, "places_ss"] - 1
        places = places_mat_poss.loc[work_ss_id, "places_ss"]
        """
        if type(places)!= np.float64:
            places = places[0]
        """
        capa_per_ss.loc[work_ss_id, "places_ss"] = places

        if  places <=0:
            possible_ss_to.drop([work_ss_id]) #, inplace=True
...
print(mat_lim)
mat_lim.to_csv('sec_limitrophe.csv')
print(capa_per_ss)
mat_workplace = pd.concat([mat_out, mat_lim], ignore_index = True)

from_to_commune = pd.read_csv("sec_com_flow.csv", sep=';')
...
for com in communes:
    print("com", com)
    for co in communes:
        nb_already = len(mat_workplace[(mat_workplace.Commune_x == com) & (mat_workplace.WorkCommuneName == co)])
        should = str(from_to_commune.loc[com, co])
        should = should.replace(".", "")
        should = int(should)
        
        possible = maternelles[maternelles.Commune_x == com]
...
print(mat_com)
mat_com.to_csv("sec_commune.csv")
...
capa_per_ss = capa_per_ss[capa_per_ss["places_ss"] > 0]
mat_random =  pd.DataFrame(columns=colnames)
i = 0
if not maternelles.empty and not capa_per_ss.empty:
    possibles_ss = capa_per_ss[capa_per_ss["places_ss"] > 0]
    ...
    while len(possibles_ss) > 0 and len(maternelles) > 0:
        ...
        capa_per_ss.loc[work_ss_id, "places_ss"] = capa_per_ss.loc[work_ss_id, "places_ss"] - 1
        places = capa_per_ss.loc[work_ss_id, "places_ss"]
        """
        if type(places)!= np.float64:
            places = places[0]
        """
...
print(mat_workplace)
mat_workplace.to_csv("secondaires_workplace.csv")
        
print(capa_per_ss)
capa_per_ss.to_csv("sec_places_restantes_for_foreigners.csv")
