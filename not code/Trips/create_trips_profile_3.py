...
people =  pd.read_csv('all_pers_type_3.csv')# TODO LOAD pers and filter tripChainType 1
print(people)
...
def trip_to_loisir(ss_from, end_time_to_previous_pos, delta_t):
    ...
    if end_time_to_previous_pos >= 1337: #only null probability after
        end_time_to_previous_pos = 0
    
    count = 0
    while start_time_to_loisir_pos < end_time_to_previous_pos and count < 1442:
        count += 1
        print('out', end_time_to_previous_pos)
...
for pers in people.index:
    ...
    if ss_home != ss_work:
        ...
        distance, start_time_to_home_pos, start_time_to_home_hour, start_time_to_home_min, end_time_to_home_pos, end_time_to_home_hour, end_time_to_home_min, delta_t_to_home = trip_to_home(ss_work, end_time_to_work_pos, 8*60)
