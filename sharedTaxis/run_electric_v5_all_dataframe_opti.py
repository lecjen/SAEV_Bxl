...
delta_t_ori_destis = pd.DataFrame(columns=time_to.columns.tolist())
...
for delta in range(0, 40):
    ...
    for col in final.columns.tolist():
        ...
        delta_t_ori_destis.loc[delta, col]= r

print(delta_t_ori_destis)
...
print(distances) #TODO check format
"""
delta_t_ori_destis = pd.read_csv("delta_t_ori_destis.csv")
print(delta_t_ori_destis) #TODO check format
"""
...
save_requests.to_csv('requests_test_3.csv')
"""
requests = pd.read_csv('requests_test_3.csv')
requests.sort_values(by='time', inplace = True)
...
for t in range(0, 1441): #TODO check max departure time de mes trips
    ...
    #r = requests[requests.time <= t+30]
    if not taxis_available.empty and not requests.empty: # r.empty:
        orig = taxis_available.pos
        ...
        for delta in range(0, 1441-t):
            for delta_waiting in range(0, max_waiting_time+1):
                for o in orig:
                    
                    needed_t = delta+delta_waiting
                    if needed_t > 39:
                        needed_t = 39
                        #break
                        
                    possibles_desti = delta_t_ori_destis.loc[needed_t, o]
                    
                    possibles_requests = requests[(requests.time == delta+delta_waiting) & (requests.origin.isin(possibles_desti))]
                    if not possibles_requests.empty:
                        possible_taxis = taxis_available[taxis_available.pos == o]
                        
                        while len(possible_taxis)>0 and len(possibles_requests)>0:
                            elu = random.randrange(0, len(possible_taxis), 1)
                            taxi = possible_taxis.index[elu]
                
                            elu = random.randrange(0, len(possibles_requests), 1)
                            ind = possibles_requests.index[elu]
                            ...
                            taxis.loc[taxi, "book_time"] = delta+delta_waiting + t
                            if delta+delta_waiting + t > requests.loc[ind, "time"]:
                    
                                taxis.loc[taxi, "tot_waiting_time"] += (delta+delta_waiting + t -  requests.loc[ind, "time"]) * taxis.loc[taxi, "nb_pers"]
                                if ((delta+delta_waiting + t -  requests.loc[ind, "time"]) * taxis.loc[taxi, "nb_pers"]) <0:
                                    stop
                                taxis.loc[taxi, "departure_time"] = delta+delta_waiting + t
                            ...
                            possibles_requests.drop([ind], inplace=True)   
                            ...
                            for delta_t in range(int(taxis.loc[taxi, "departure_time"]), taxis.loc[taxi, "max_departure_time"] - t +1):
                                ...
                                while len(compagnons_request) > 0 and taxis.loc[taxi, "nb_pers"] < max_places:
                                    ...
                                    compagnons_request.drop([ind_elu], inplace=True)
                                    requests.drop([ind_elu], inplace=True)
                                    possibles_requests.drop([ind_elu], inplace=True) 
...
print(taxis)
taxis.to_csv('taxis_simulation_3.csv')
print(stations)
stations.to_csv('stations_simulation_3.csv')      
