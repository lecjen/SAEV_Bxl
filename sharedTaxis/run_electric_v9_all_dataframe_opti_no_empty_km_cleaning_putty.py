...
def int_na(x):
    if x == 'NaN' or x == float('NaN') or x == np.NaN or x == np.nan or np.isnan(x):
...
taxis = pd.read_csv('big_simu_putty_taxis.csv')
taxis.status_time = taxis.status_time.apply(lambda x : list(x.split(",")))
taxis.nb_pers = taxis.nb_pers.apply(lambda x : int_na(x))
taxis.destination_time = taxis.destination_time.apply(lambda x : int_na(x))
taxis.book_time = taxis.book_time.apply(lambda x : int_na(x))
taxis.departure_time = taxis.departure_time.apply(lambda x : int_na(x))
taxis.max_departure_time = taxis.max_departure_time.apply(lambda x : int_na(x))
taxis.km_battery = taxis.km_battery.apply(lambda x : int_na(x))
taxis.tot_charging_time = taxis.tot_charging_time.apply(lambda x : int_na(x))
taxis.book_charge_desination_time = taxis.book_charge_desination_time.apply(lambda x : int_na(x))
taxis.tot_charging_time = taxis.tot_charging_time.apply(lambda x : int_na(x))
taxis.cleaning_place = taxis.cleaning_place.apply(lambda x : int_na(x))
taxis.charging_station = taxis.charging_station.apply(lambda x : int_na(x))
taxis.time_cleaning_done = taxis.time_cleaning_done.apply(lambda x : int_na(x))
taxis.tot_cleaning_time = taxis.tot_cleaning_time.apply(lambda x : int_na(x))
taxis.drop(columns=['Unnamed: 0'], inplace = True)

requests = pd.read_csv("big_simu_putty_request.csv") #read from trips files mais ordonnés par time departure croissant
...
taxis = pd.DataFrame(columns = ["status", "nb_pers", "pos", "destination_book", "destination_working", "destination_time", "book_time", 
                                ...
                                "status_time", "destination_charge", "cleaning_place", "time_cleaning_done", "tot_cleaning_time"])
"""
l = []
...
stations = pd.read_csv('big_simu_putty_stations.csv')
stations.drop(columns=['Unnamed: 0'], inplace = True)
stations.status_time = stations.status_time.apply(lambda x : list(x.split(",")))

cleaning_place = pd.DataFrame(columns=["position", "status", "status_time", "tot_nb_cleaning"])
#cleaning_place = pd.read_csv('big_simu_putty_cleaning.csv')

#cleaning_place.status_time = cleaning_place.status_time.apply(lambda x : list(x.split(",")))
...
for t in range(316, 1441): #TODO check max departure time de mes trips
    print("t", t)
    ...
    #elif taxis.loc[taxi, "status"] == "cleaning": and if taxis.loc[taxi, "km_battery"] == max_km_per_charge:
    mask_cleaning_done = ((taxis.status == "cleaning") & (taxis.time_cleaning_done == t))
    taxis_cleaning_done = taxis[mask_charging_done]
    ...
    mask_cleaning = ((taxis.status == "cleaning"))
    #taxis_charging = taxis[mask_charging]
    taxis.loc[mask_cleaning, "tot_cleaning_time"] += 1
    print("cleaning time done")
            ...
            for ori in origins:
                ...
                while len(filtered_requests) > 0:
                    ...
                    if not available_taxis.empty:
                        ...
                        new = True
                        new_taxi = ["waiting", 1, ori, ori, r.SS_Destination, None, t, t, t + max_waiting_time, max_km_per_charge, None, 0, 0, 0, 0, 0, 0, 0, None, 0, l.copy(), None, None, None, 0] #Taxi("waiting", ori)
                    #"status", "nb_pers", "pos", "destination_book", "destination_working", "destination_time", "book_time", 
                    #                "departure_time", "max_departure_time", "km_battery", "charging_station", "tot_waiting_time", "tot_empty_km",
                    #                "tot_pers_km", "pers_per_trip", "nb_trips", "km", "tot_charging_time", "cleaning_place", "time_cleaning_done", "tot_cleaning_time"
                    """
                    new_taxi.nb_pers +=1
    ...
    """
    taxis_to_clean = taxis[(taxis.status == "free") & (taxis.nb_trips%40 == 0)]
    for taxi in taxis_to_clean.index:
        #if taxis.loc[taxi, "status"] == "free" and taxis.loc[taxi, "km_battery"] < max_km_per_charge:
            
        free_cleaning_places = cleaning_place[(cleaning_place.status == "free") & (cleaning_place.position == taxis.loc[taxi, "pos"])]
        if len(free_cleaning_places)>0:
            ind_cleaning = free_cleaning_places.index[0]
            
            #taxis.loc[taxi, "destination_charge"] = ss_int
            taxis.loc[taxi, "status"] = "cleaning"
            #taxis.loc[taxi, "book_charge_desination_time"] = t
            cleaning_place.loc[ind_cleaning, "status"] = "busy"
            cleaning_place.loc[ind_cleaning, "tot_nb_cleaning"] += 1
            taxis.loc[taxi, "cleaning_place"] = ind_cleaning
            taxis.loc[taxi, "time_cleaning_done"] = t + cleaning_time
    ...
    taxis.to_csv('big_simu_putty_taxis.csv')
    requests.to_csv('big_simu_putty_request.csv')
    stations.to_csv('big_simu_putty_stations.csv')
    cleaning_place.to_csv('big_simu_putty_cleaning.csv')
