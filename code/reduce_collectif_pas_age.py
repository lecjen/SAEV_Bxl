...
repart = pd.read_csv('population.csv', sep=';')
...
for ss in sectors:
    print("ss", ss)
    nb_fe = repart["Nombre de femmes"][ss]

    currently_fe = pop[(pop.SectorStatID == ss) & (pop.GenderID == 1)]
    nb_fe_too_many = len(currently_fe)-nb_fe
    possible_fe = pop[(pop.SectorStatID == ss) & (pop.GenderID == 1) & (pop.HouseholdTypeID == 6)]
    ...
    while nb_fe_too_many > 0 and len(possible_fe) > 0:
        ...
        pop.drop([ind_elu], inplace=True)

        nb_fe_too_many -=1
        j+=1
        
    
    nb_ho = repart["Nombre d'hommes"][ss]

    currently_ho = pop[(pop.SectorStatID == ss) & (pop.GenderID == 0)]
    nb_ho_too_many = len(currently_ho)-nb_ho
    possible_ho = pop[(pop.SectorStatID == ss) & (pop.GenderID == 0) & (pop.HouseholdTypeID == 6)]
