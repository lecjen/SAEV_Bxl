...
places = pd.read_csv('mat_places_restantes_for_foreigners.csv')
places.set_index("SectorStatID", inplace = True, drop = True)
places = places[places["Places Mat ss"]> 0]
print(places)
...
for ss in places.index:
    ...
    p= places.loc[ss, "Places Mat ss"]
    while p > 0:
        print("p", p)
        baby.loc[i]=[ss_from_id, ss_from_name, i, "Useless for non resident", "Useless for non resident", "Useless for non resident", "Useless for non resident", "Useless for non resident", "Useless for non resident", 1, "Maternelles", ss, ss_to_name]
        p-=1
        i+=1

print(baby)
baby.to_csv("foreign_mat_workplace.csv")